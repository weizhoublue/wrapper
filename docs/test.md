...
# 4. 错误命令处理（期望退出码 204）
node src/main.js -t opencode -c "opencode-bad" -d \
  -p "what is your name"
echo $?

# 5. 正则输出匹配
node src/main.js -t opencode -d \
  -e "bingo|Bingo" \
  -p "please say bingo in english"  2>/tmp/a
echo $?
echo ""
cat /tmp/a
...
# 6. 重试控制（相同会话内进行重试）
node src/main.js -t opencode -d -r 2 \
  -e "no_bingo" \
  -p "say hi in one word ? reply me in english"
echo $?
...
# 7. 超时控制（期望超时退出码 203）
node src/main.js -t opencode -d -o 1 \
  -p "please reply me after 10 seconds"  2>/tmp/a
echo $?
echo ""
cat /tmp/a
...
# 8. 会话恢复 (Resume)
node src/main.js -t opencode -p "tomorrow is monday" -d 2>/tmp/sid
session=$(tail -1 /tmp/sid)
cat /tmp/sid
echo "session=${session}"
echo "------------------"
...
# 9. 重试过程中的会话一致性
node src/main.js -t opencode -p "what is your name" -d 2>/tmp/sid
session=$(tail -1 /tmp/sid)
echo "session=${session}"
echo "------------------"
node src/main.js -t opencode -d \
